...
window.addSingleLocation = async () => {
    ...
    try {
        await setDoc(doc(db, LOC_COLLECTION, newId), { dong: '', pos: '', code: '', name: '', option: '', stock: '0', updatedAt: new Date() }, { merge: true });
...
window.openEditModal = (id) => {
    ...
    document.getElementById('modal-dong').value = target.dong || '';
    document.getElementById('modal-pos').value = target.pos || '';
...
async function updateDatabase(rows) {
    const tbody = document.getElementById('location-list-body');
    if (tbody) tbody.innerHTML = '<tr><td colspan="8" style="padding:50px; font-weight:bold; color:#3d5afe;">초고속 동기화 진행 중...</td></tr>';
